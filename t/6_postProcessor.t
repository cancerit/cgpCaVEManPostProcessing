...
subtest 'getNormIndelReadDepthResult' => sub{
  ...
  ok($processor->getNormIndelReadDepthResult == 1,"Pass norm indel filter 11% changed proportion");
  done_testing();
};
...
subtest '_getDistanceFromGapInRead' => sub {
  ...
  my $res = Sanger::CGP::CavemanPostProcessor::PostProcessor::_getDistanceFromGapInRead(\@cig_array, $rd_pos_of_interest);
  ok($res==0, "Check deletion is recorded at position of interest. $res!=0");
  @cig_array = ( ['M', 8], ['I', 1], ['M', 7], );
  $res = Sanger::CGP::CavemanPostProcessor::PostProcessor::_getDistanceFromGapInRead(\@cig_array, $rd_pos_of_interest);
...
subtest '_isCurrentPosCoveredFromAlignment Tests' => sub {
  ...
  my $currentPos = 9;
  my $res = Sanger::CGP::CavemanPostProcessor::PostProcessor::_isCurrentPosCoveredFromAlignment($pos, \@cig_array, $currentPos);
  ok($res==1, "Position is covered 1 before deletion $res != 1");
  $currentPos = 10;
  $res = Sanger::CGP::CavemanPostProcessor::PostProcessor::_isCurrentPosCoveredFromAlignment($pos, \@cig_array, $currentPos);
